...
static int process_image_descriptor( int gif_file, rgb *gct, int gct_size, int resolution_bits, gif_image_t* image ) {
  ...
  if (blockListEntry->block.image_descriptor.fields && 0x80) {
    ...
    if ( read( gif_file, blockListEntry->block.local_color_table, 3 * blockListEntry->block.local_color_table_size ) < 3 * blockListEntry->block.local_color_table_size ) {
  ...
  blockListEntry->block.decoded_data = (unsigned char *) malloc( blockListEntry->block.data_length );
...
static int process_extension( int gif_file, gif_image_t* image ) {
  block_list_t* blockListEntry = addBlockListEntry(image);
  ...
  extension_data_length = read_sub_blocks( gif_file, &blockListEntry->extensionHeader.data );
...
bool process_gif_stream( int gif_file, gif_image_t* image) {
  ...
  if ( image->screen_descriptor.fields & 0x80 ) {
    ...
    // XXX this could conceivably return a short count...
    if ( read( gif_file, image->global_color_table, 3 * image->global_color_table_size ) < 3 * image->global_color_table_size ) {
